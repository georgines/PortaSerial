...
public:
    ...
    PortaSerial(uart_inst_t *uart_escolhida, uint32_t taxa_baud, uint pino_tx, uint pino_rx);
    ...
    bool enviarValor(const TipoValor &valor) {
        ...
        int quantidade_formatada = std::snprintf(buffer, TAMANHO_BUFFER_VALOR, "%g", static_cast<double>(valor));
